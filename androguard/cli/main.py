...
from Util import getPermissionAndApi, getSenstiveApi,featOneHot,getPermissionTypes
...
def graphCut(cg,entry_points):
    perType = getPermissionTypes()
    (Api,Apermissions) = getSenstiveApi()
    #Api = getPermissionAndApi()
    SaveList = []
    #遍历图的所有节点，将非敏感的节点（或非敏感节点路径）删除
    for node in cg.nodes():
        (cg.nodes[node]['sensitive'],is_sen) = featOneHot(node.class_name + node.name, (Api, Apermissions), perType)
        if is_sen:
            if node not in SaveList:
                SaveList.append(node)
            PreAndSuc(cg,node,SaveList)
    ...
    for item in cg.nodes(data=True):
        nfeatures.append(item[1])
    return nfeatures



def PreAndSuc(cg,node,SaveList):
    for pre in cg.predecessors(node):
        if pre in SaveList:
            continue
        else:
            SaveList.append(pre)
    for suc in cg.neighbors(node):
        if suc in SaveList:
            continue
        else:
            SaveList.append(suc)
    ...
    # allDrop = True
    # repeat = False
    # if node.class_name+node.name in Api:
    #     return False
    # for succ in cg.neighbors(node):
    #     if succ in CheckList:
    #         #重复访问，即有循环，跳过
    #         continue
    #     CheckList.append(succ)
    #     if len(list(cg.neighbors(succ))) == 0:
    #         if succ.class_name+succ.name not in Api:
    #             DropList.append(succ)
    #         else:
    #             allDrop = False
    #     elif R(cg,succ,Api,DropList,CheckList):
    #         if succ not in DropList:
    #             DropList.append(succ)
    #     else:
    #         allDrop = False
    # return allDrop
...
def androcg_main(verbose,
    ...
    nfeatures = graphCut(CG,entry_points)
    print('after' + str(CG.order()))
